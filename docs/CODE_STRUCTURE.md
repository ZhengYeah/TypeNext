...
This report describes the working tree inspected on 2026-09-20: 57 existing tracked files, comprising 26 implementation Go files, 17 Go test files, and 14 supporting files. This new report and the ignored, generated `TypeNext.exe` are additional files. Existing local edits are reflected in the descriptions. Four tracked test files are currently deleted; they are identified near the end rather than counted as active tests.
...
The application is organized into three Go packages:
...
| `internal/win` | Windows UI, keyboard/mouse hooks, accessibility readers, preview, insertion, and encrypted key storage. | `internal/core` |
| `internal/core` | Portable configuration, endpoint policy, model requests, text processing, and shortcut management. | None |

`cmd/typenext` and `internal/win` Go files require the `windows && amd64` build target. `internal/core` can be built and tested on other operating systems. The files inside `internal/win` share one package and one application state object; the filenames divide responsibilities without creating separate Go packages.
...
flowchart TD
    ...
    Request --> Capture[uiaWorker.CaptureInitial]
...
| `win.uiaWorker` / `win.caretIdentity` | Serialized accessibility jobs and the logical caret baseline used to verify one request. |
...
| `app.accept()` | Require a finished candidate and the expected foreground window, hide the preview, and invoke `uiaWorker.Insert()` asynchronously. |
...
| `app.inspect()` | Capture the focused textbox and display local diagnostic text/context without calling the model. |
...
Accessibility and PowerPoint support have separate files because text extraction, logical caret identity, and visual popup position solve different problems:

| File | Feature | Main functions and their targets |
|---|---|---|
| [uia_windows.go](../internal/win/uia_windows.go) | Accessibility worker, main context capture, and final insertion checks. | `newUIA()` initializes the dedicated COM worker. `uiaWorker.CaptureInitial()` starts a fresh identity baseline; `Capture()` retains it for verification; `capture()` queues work and waits with cancellation. `readContext()` enforces application approval and safe focused-control/selection checks, reads bounded text, determines identity/position, and rechecks focus. `Insert()` waits for key release, verifies revision/context again, sanitizes text, and calls `sendUnicode()`. |
| [uia_windows.go](../internal/win/uia_windows.go), COM helpers | Shared COM calling and UIA range utilities. | `comCall()` invokes a COM vtable slot. `failed()` checks HRESULT failure; `release()` manages COM references. `scalar()` reads scalar properties; `pattern()` obtains an interface. `cloneRange()`, `moveEnd()`, and `rangeReadonly()` support range handling. `safeArrayData()` accesses checked SAFEARRAY contents. `focusID()` reads UIA runtime identity; `rangeRect()` extracts range geometry. Defines `comObject` and `variant`. |
| [uia_client_windows.go](../internal/win/uia_client_windows.go) | UI Automation client creation and native timeouts. | `createAutomationClass()` wraps `CoCreateInstance`. `createUIAutomation()` creates the newer automation client with 1,000 ms connection and 1,500 ms transaction timeouts. It falls back to the legacy client only when the newer class/interface is unavailable. |
| [uia_context_windows.go](../internal/win/uia_context_windows.go) | Bounded text adjacent to the caret and safe selection reads. | `boundedRangeText()` reads bounded UTF-16 text and reports whether the requested range was complete. `readCaretSide()` reads prefix/suffix ranges while preserving adjacency to the caret when providers promote character movement to larger units. `collapsedSelection()` requires exactly one empty selection. `verifyCaretSelection()` rechecks that the selection/caret still matches after reading. |
| [uia_identity_windows.go](../internal/win/uia_identity_windows.go) | Caret identity independent of popup coordinates. | `caretIdentity.identifyInPattern()` selects an exact offset or retained-range identity and keeps the chosen scheme stable during a request. `identify()` compares/clones retained ranges and advances a serial when the caret changes. `compareRangeEndpoints()` compares both endpoints with explicit errors; `sameRangeEndpoints()` returns a boolean. `close()` releases the retained baseline. |
| [uia_position_windows.go](../internal/win/uia_position_windows.go) | Stable logical caret offsets across provider recreation. | `stableCaretOffset()` moves a cloned range toward document start and verifies the round trip to the original caret. It uses at most 8,192 character units, reads no text, and returns failure when exact positioning cannot be established. Despite the filename, this handles logical position rather than screen coordinates. |
| [uia_geometry_windows.go](../internal/win/uia_geometry_windows.go) | Visual caret-position fallback. | `adjacentCaretPosition()` estimates a popup anchor from a character range next to the caret when the empty caret range has no usable rectangle. It respects cancellation. |
...
For a normal accessible textbox, `readContext()` requires an approved application, no active IME composition, a focused/enabled non-password Edit or Document control, and a collapsed selection. It rejects text reported read-only and prefers a matching active TextPattern2 caret when available. It reads context, rechecks the focused element and selection, then establishes caret identity.
...
| [internal/win/com_windows_test.go](../internal/win/com_windows_test.go) | `TestCOMCallArguments`: validates COM argument forwarding and HRESULT handling with fake vtables. `BenchmarkCOMCall`: measures wrapper overhead. |
| [internal/win/pad_windows_test.go](../internal/win/pad_windows_test.go) | `TestPadAccessibilityText`: creates a real Windows test control and checks UIA text-pattern access, prefix/suffix reads, Unicode, and selection state. `testPadControl()` / `readTestPadRanges()` provide native-control helpers. |
...
| [internal/win/uia_client_windows_test.go](../internal/win/uia_client_windows_test.go) | `TestUIAutomationInitialization`: client creation/fallback behavior with fake COM objects. `TestUIAutomationNativeTimeouts`: native automation-client timeout settings. |
| [internal/win/uia_context_windows_test.go](../internal/win/uia_context_windows_test.go) | `TestReadCaretPrefixKeepsCaretAdjacencyWithPromotedUnits`, `TestReadCaretPrefixRejectsOversizedSingleUnit`, and `TestReadCaretSuffixAllowsFarEndTruncation`: text boundaries with provider-specific movement. Also covers zero context, Unicode/embedded NUL, cancellation, and `TestVerifyCaretSelectionAfterRead`. |
| [internal/win/uia_geometry_windows_test.go](../internal/win/uia_geometry_windows_test.go) | `TestUIACaretFingerprintWithoutPosition`: identity independent of the visual anchor. `TestAdjacentCaretPosition` and `TestAdjacentCaretPositionCancellation`: geometry fallback and cancellation with fake ranges. |
| [internal/win/uia_identity_windows_test.go](../internal/win/uia_identity_windows_test.go) | `TestCaretIdentityAcrossCaptures`: stable retained-range identities. `TestCaretIdentityPreservesSnapshotOnProviderFailure` and `TestCaretIdentityRejectsMovementAfterProviderFailure`: retain a baseline on temporary errors without accepting later movement. `TestCaretComparisonRequiresBothEndpoints`: compare the entire collapsed range. |
| [internal/win/uia_position_windows_test.go](../internal/win/uia_position_windows_test.go) | `TestStableCaretOffsetAcrossProviderRecreation` and `TestStableCaretOffsetDistinguishesRepeatedTextPositions`: offset identity. `TestStableCaretOffsetRejectsUnverifiedPositions`, `TestStableCaretOffsetLimitExhaustion`, and `TestStableCaretOffsetRejectsMissingInterfaces`: conservative failure paths. `TestStableCaretOffsetWithWindowsProvider`: verifies offset behavior against a real Windows editor control. |
...
| Change how a focused textbox is read | `internal/win/uia_windows.go` and `uia_context_windows.go`. |
| Fix stale-caret or repeated-text identity behavior | `internal/win/uia_identity_windows.go`, `uia_position_windows.go`, and `internal/core/text.go`. |
| Change popup placement or appearance | `internal/win/uia_geometry_windows.go`, `native_windows.go`, and `app_windows.go` (`showOverlay()` / `paintOverlay()`). |
| Change PowerPoint support | `internal/win/powerpoint_windows.go`. |
| Change acceptance, cancellation, or failure behavior | `internal/win/app_windows.go`, `suggestion_windows.go`, `uia_windows.go`, and `native_windows.go`. |
...
The runtime checks have two practical boundaries relevant to understanding the design. Accessibility depends on each application's provider and the dedicated PowerPoint adapter. Final verification followed by `SendInput` is not an atomic edit transaction across processes, so a small focus/edit race remains. These are existing implementation constraints, not failures discovered by this documentation task.

Validation for this report is a source/inventory review. No application source was changed, and no runtime test results are claimed. For project verification, the Windows build script runs all current tests and static analysis; the shell script runs portable tests and cross-build checks. Live application compatibility and actual model behavior require separate Windows testing.
