...
| [api_windows.go](../internal/win/api_windows.go) | API configuration interface and endpoint approval. | `app.openAPI()` / `closeAPI()` manage the dialog. `apiConnectionFields()` reads manually entered connection fields. `newAPIModel()` starts a new connection with empty name, endpoint, model, and key fields. `updateAPIKeyStatus()` describes saved-key availability for the draft endpoint. `approveRemote()` obtains approval for the exact request URL. `saveAPI()` validates fields, manages saved keys, persists settings, and updates active state. `connectionSummary()` / `refreshConnectionUI()` update the main connection description; `apiMessage()` displays dialog status. `setControlText()`, `setCheck()`, `comboIndex()`, and `selectCombo()` manipulate native controls. |
...
| Change connection settings | `internal/core/config.go`, `endpoint.go`, `client.go`; API interface in `internal/win/api_windows.go`. |
